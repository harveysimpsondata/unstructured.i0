...
def extract_content(filename, chunking_strategy, multipage_sections=True, similarity_threshold=0.5, hi_res_model_name='layout_v1.1.0', max_characters=500):
...
def main():
    ...
    chunking_strategies = ['basic', 'by_title', 'by_similarity']
    multipage_sections = True  # Set based on document needs
    similarity_threshold = 0.9  # Relevant for 'by_similarity'
    #hi_res_model_name = 'yolox'  # Set the high-resolution model name
    hi_res_model_name = 'layout_v1.1.0'
    max_characters = 500  # Limit the number of characters in each chunk

    for strategy in chunking_strategies:
        extracted_data = extract_content(filename, strategy, multipage_sections, similarity_threshold, hi_res_model_name, max_characters)

        if extracted_data is None:
            print(f"Failed to extract content using {strategy} chunking.")
        else:
            save_json_to_folder(extracted_data, folder_path, filename, strategy, hi_res_model_name)
            print(f"JSON output saved successfully to {folder_path} with {strategy} chunking and {hi_res_model_name} model")
